...
def _get_division_schedule(division: str) -> dict:
    """Returns schedule grouped by day for a division (student view)"""
    if division == PLACEHOLDER_DIV or division not in all_divisions:
        return {day: [] for day in DAY_ORDER}
...
.role-grid { display: flex; gap: 1rem; margin: 1.25rem 0 1.25rem 0; }
.role-card { flex: 1; background: var(--white); border: 1px solid var(--line); border-radius: 12px; padding: 2rem; display:flex; align-items:center; justify-content:center; cursor:pointer; box-shadow: 0 8px 20px -8px rgba(43,7,7,0.05); }
.role-card:hover { box-shadow: 0 12px 30px -10px rgba(43,7,7,0.08); transform: translateY(-2px); }
.role-card h3 { margin:0; font-family: 'Fraunces', serif; font-size:1.25rem; color: var(--maroon-900); }
...
if "student_division" not in st.session_state:
    st.session_state["student_division"] = PLACEHOLDER_DIV
if "student_detail_day" not in st.session_state:
    st.session_state["student_detail_day"] = None


# ════════════════════════════════════════════════════════════════
# ROLE SELECTION (student / professor)
# This is shown only when role is not yet chosen.
# ════════════════════════════════════════════════════════════════
if st.session_state["role"] is None:
    st.markdown('<div style="margin-top:1.25rem; margin-bottom:0.5rem; font-weight:600; font-size:1.05rem;">What describes you</div>', unsafe_allow_html=True)
    c1, c2 = st.columns([1, 1], gap="large")
    with c1:
        # Student card
        if st.button("👩‍🎓 Student", key="choose_student", use_container_width=True):
            st.session_state["role"] = "student"
            st.session_state["student_division"] = PLACEHOLDER_DIV
            st.experimental_rerun()
        st.markdown('<div style="text-align:center; color:var(--ink-faint); margin-top:0.5rem;">Select this if you are a student</div>', unsafe_allow_html=True)
    with c2:
        # Professor card
        if st.button("👨‍🏫 Professor", key="choose_professor", use_container_width=True):
            st.session_state["role"] = "professor"
            st.experimental_rerun()
        st.markdown('<div style="text-align:center; color:var(--ink-faint); margin-top:0.5rem;">Select this if you are a faculty member</div>', unsafe_allow_html=True)

# If role not chosen, stop here so professor panel doesn't render
if st.session_state["role"] is None:
    st.stop()


# If student flow is active, render student UI
if st.session_state["role"] == "student":
    with st.container(border=True):
        ...
        col1, _ = st.columns([2, 3], gap="small")
        with col1:
            st.markdown(f'<div class="field-label">Division</div>', unsafe_allow_html=True)
            div_options = [PLACEHOLDER_DIV] + all_divisions
        ...
        # When division selected, show 5-day summary
        if st.session_state["student_division"] != PLACEHOLDER_DIV:
            schedule = _get_division_schedule(st.session_state["student_division"])

            # Back button to choose role
            if st.button("← Back to role selection", key="student_back"):
                st.session_state["role"] = None
                st.session_state["student_division"] = PLACEHOLDER_DIV
                st.session_state["student_detail_day"] = None
                st.experimental_rerun()
            ...
            for day in DAY_ORDER:
                ...
                <div class="day-section">
                  <div class="day-header">
                    <span class="day-emoji">{'🌅' if day=='Monday' else '☀️' if day=='Tuesday' else '🌤️' if day=='Wednesday' else '⛅' if day=='Thursday' else '🌆'}</span>
                    <span class="day-title">{day}</span>
                    <span class="day-count">{len(sessions)} session{'s' if len(sessions) != 1 else ''}</span>
                  </div>
                """, unsafe_allow_html=True)
                ...
                if not sessions:
                    ...
                    # show a short summary with a button to see details
                    if st.button(f"Click to see detailed schedule — {day}", key=f"student_view_{day}"):
                        st.session_state["student_detail_day"] = day
                        st.experimental_rerun()

                    # If this day is expanded, show detailed sessions
                    if st.session_state["student_detail_day"] == day:
                        # Detailed view: list sessions one by one
                        for idx, session in enumerate(sessions):
                            st.markdown(f"""
                            <div class="detailed-card type-theory">
                              <div class="detailed-top">
                                <span class="detailed-chip chip-theory">{escape_html(session.get('type',''))}</span>
                              </div>
                              <div class="detailed-subject">{escape_html(session.get('subject',''))}</div>
                              <div class="detailed-meta">
                                <div>
                                  <div class="meta-label">Time</div>
                                  <div class="meta-value">{escape_html(session.get('time_slot',''))}</div>
                                </div>
                                <div>
                                  <div class="meta-label">Room</div>
                                  <div class="meta-value">{escape_html(session.get('room',''))}</div>
                                </div>
                                <div>
                                  <div class="meta-label">Professor</div>
                                  <div class="meta-value">{escape_html(session.get('professor',''))}</div>
                                </div>
                              </div>
                            </div>
                            """, unsafe_allow_html=True)

                        if st.button("← Back to weekly view", key=f"student_back_week_{day}"):
                            st.session_state["student_detail_day"] = None
                            st.experimental_rerun()
...
# ════════════════════════════════════════════════════════════════
# PROFESSOR SELECTION PANEL
# (Unchanged workflow — rendered only when role == "professor")
# ════════════════════════════════════════════════════════════════
prof_options = [PLACEHOLDER_PROF] + all_professors
selected_prof = st.session_state["prof_select"]
...
# ════════════════════════════════════════════════════════════════
# TIMETABLE VIEW (professor flow continues unchanged)
# ════════════════════════════════════════════════════════════════
